function skin3D(path, elem) {
	...
	this.createCloak  = function(cloakPath) {
		this.cloakTexture = new THREE.TextureLoader().load(cloakPath);
		console.log(this.cloakTexture);
		this.cloakTexture.magFilter = THREE.NearestFilter;
		this.cloakTexture.minFilter = THREE.NearestMipMapNearestFilter;
		let cloak = new THREE.BoxGeometry(10, 16, 1, 0, 0, 0);

		let _this = this;
		this.cloakTexture.image.onload = function()
		{
			let wMax = 22, hMax = 17, addHeight = 0;
	...
	this.changeSkinName = function(nickname) {
		let partPath = this.img.src.split("/"),
			newpath = "";
	...
	this.changeCloak = function(newpath) {
		if (cloakMesh == null) return false;
		this.removeCloak();
	...
	this.addEvents = function() {
		...
		renderElem.onmousedown = function(e) {
			let sX = _this.MouseCoords.getX(e),
		...
		renderElem.onwheel = function(e) {
			e = e || window.event;
			let val = e.deltaY || e.detail || e.wheelDelta;
	...
	this.animate = function() {
		...
		setInterval(function()
		{
			...
			let b = _this.radius * Math.cos(_this.beta);
	...
	this.render = function() {
		...
		this.rendered.setClearColor(0x000000, 0);
	};
	...
	this.GetCTX = function(canvas)
	{
		...
		let names = ["2d", "webgl", "experimental-webgl", "webkit-3d", "moz-webgl"];
		let ctx = null;
...
let skin = new skin3D("https://slymc.42web.io/88a18730-3d85-11ec-b732-854fb897c640.png", document.getElementById("skin"));
skin.width = 304;
skin.height = 417;
skin.radius = 30;
skin.init();

skin.createCloak("https://slymc.42web.io/cloak.png");
skin.loadFail = function(errorCode) {
	alert("произошла ошибка #" + errorCode);
};
