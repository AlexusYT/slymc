...
class Utils
{
	...
	public static function getDb(): SafeMySQL
	{
		if(!isset($GLOBALS["sqldb"])) {
			$GLOBALS["sqldb"] = new SafeMySQL(['host' => '82.202.160.186', 'port' => '3316', 'user' => 'u244065_api', 'pass' => 'pL3gT1fU4l', 'db' => 'u244065_slymc_main', 'charset' => 'utf8mb4']);
	...
	*|\[(?:IPv6:(?:[a-f0-9]{1,4}(?::[a-f0-9]{1,4}){7}|(?!(?:.*[a-f0-9][:\]]){7,})(?:[a-f0-9]{1,4}(?::[a-f0-9]{1,4}){0,5})?::
	...
	public static function getDefaultHead():string
	{
		if(!isset($GLOBALS["defaultPlayerHead"])){
			$GLOBALS["defaultPlayerHead"] = base64_encode(file_get_contents("textures/steveHead.png", true));
		}
		return $GLOBALS["defaultPlayerHead"];
	}

	private static function getSavedUsers(){
		return json_decode(file_get_contents("savedUsers.json", true), true)["users"];
	}
